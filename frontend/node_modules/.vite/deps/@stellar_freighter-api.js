  ...
  "node_modules/@stellar/freighter-api/build/index.min.js"(exports, module) {
    !function(r, e) {
      ...
      const n = (r2) => {
        ...
        if (I) {
          const t2 = await (async (r3, s3, t3) => {
            let o2;
            const T2 = (t3 || {}).address;
            try {
              o2 = await n({ blob: r3, accountToSign: T2, apiVersion: s3, type: e.SUBMIT_BLOB });
            } catch (r4) {
              return { signedMessage: null, signerAddress: "", error: E };
            }
            const { signedBlob: A2, signerAddress: a2 } = o2;
            return { signedMessage: A2 || null, signerAddress: a2, error: null == o2 ? void 0 : o2.apiError };
          })(r2, "3.1.0", s2);
          return t2.error ? { signedMessage: null, signerAddress: "", error: t2.error } : { signedMessage: t2.signedMessage, signerAddress: t2.signerAddress };
        }
        ...
        if (I) {
          const t2 = await (async (r3, s3) => {
            ...
            let o2;
            try {
              o2 = await n({ entryXdr: r3, accountToSign: t3, networkPassphrase: null == s3 ? void 0 : s3.networkPassphrase, type: e.SUBMIT_AUTH_ENTRY });
            } catch (r4) {
              return console.error(r4), { signedAuthEntry: null, signerAddress: "", error: E };
            }
            const { signedAuthEntry: T2, signerAddress: A2 } = o2;
            return { signedAuthEntry: T2 || null, signerAddress: A2, error: null == o2 ? void 0 : o2.apiError };
          })(r2, s2);
          return t2.error ? { signedAuthEntry: null, signerAddress: "", error: t2.error } : { signedAuthEntry: t2.signedAuthEntry, signerAddress: t2.signerAddress };
        }
        return { signedAuthEntry: null, signerAddress: "", error: o };
      }, c = async () => I ? window.freighter ? Promise.resolve({ isConnected: window.freighter }) : (async () => {
      ...
      })() : { isConnected: false, error: o }, N = async () => {
        if (I) {
        ...
        if (I) {
          const s2 = await (async () => {
            let r3;
            try {
              r3 = await n({ type: e.REQUEST_ALLOWED_STATUS });
            } catch (r4) {
              console.error(r4);
            }
        ...
        if (I) {
          const s2 = await (async () => {
            let r3;
            try {
              r3 = await n({ type: e.REQUEST_ACCESS });
            } catch (r4) {
              console.error(r4);
            }
            const { publicKey: s3 } = r3 || { publicKey: "" };
            return { publicKey: s3, error: null == r3 ? void 0 : r3.apiError };
          })();
          return r2 = s2.publicKey, s2.error ? { address: r2, error: s2.error } : { address: r2 };
        }
        return { address: r2, error: o };
      };
      class w {
        ...
        watch(r2) {
          return I ? (this.isRunning = true, this.fetchInfo(r2), {}) : { error: o };
        }
        stop() {
          this.isRunning = false;
        }
      }
      const I = "undefined" != typeof window, u = { getAddress: a, addToken: i, signTransaction: _, signMessage: S, signAuthEntry: d, isConnected: c, getNetwork: N, getNetworkDetails: l, isAllowed: C, setAllowed: O, requestAccess: R, WatchWalletChanges: w };
      return t;
    })());
  }
});
